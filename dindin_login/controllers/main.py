...
class DinDinLogin(Home, http.Controller):
    ...
    def get_url(self, **kw):
        ...
        new_url = "{}appid={}&response_type=code&scope=snsapi_login&redirect_uri={}".format(url, login_appid,
                                                                                            redirect_url)
        data = json.dumps(
            {"encode_url": new_url, 'callback_url': redirect_url})
        return data
        ...
        try:
            ...
            if result.get('errcode') == 0:
                ...
                employee = request.env['hr.employee'].sudo().search(
                    [('din_unionid', '=', user_info.get('unionid'))])
                if employee:
                    if employee.user_id:
    ...
    def getUseridByUnionid(self, unionid):
        """根据unionid获取userid"""
        url = request.env['ali.dindin.system.conf'].sudo().search(
            [('key', '=', 'getUseridByUnionid')]).value
        token = request.env['ali.dindin.system.conf'].sudo().search(
            [('key', '=', 'token')]).value
        data = {'unionid': unionid}
        result = requests.get(url="{}{}".format(
            url, token), params=data, timeout=20)
        logging.info(">>>根据unionid获取userid获取结果:{}".format(result.text))
        result = json.loads(result.text)
        if result.get('errcode') == 0:
            return result.get('userid')
        else:
            logging.info(">>>根据unionid获取userid获取结果失败，原因为:{}".format(
                result.get('errmsg')))
    ...
    def web_dingding_account_login(self, *args, **kw):
        ...
        url = request.env['ali.dindin.system.conf'].sudo().search(
            [('key', '=', 'sns_authorize')]).value
        appid = request.env['ir.config_parameter'].sudo(
        ).get_param('ali_dindin.din_login_appid')
        redirect_url = '{}web/dingding/account_action_login'.format(
            request.httprequest.host_url)
        new_url = '{}appid={}&response_type=code&scope=snsapi_login&state=STATE&redirect_uri={}'.format(
            url, appid, redirect_url)
