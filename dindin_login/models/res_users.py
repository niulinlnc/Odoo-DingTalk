...
class InheritResUsers(models.Model):
    ...
    din_password = fields.Char(
        string='钉钉登录密码', default=_get_defaultdin_pwd, size=64)

    def _set_password(self):
        for user in self:
            user.sudo().write(
                {'din_password': base64.b64encode(user.password.encode('utf-8'))})
    ...
    def create_user_by_employee(self, employee_id, password, active=True):
        ...
        if employee:
            ...
            email_count = len(self.search(
                [('login', 'like', email_name)]).sudo())
            if email_count > 0:
