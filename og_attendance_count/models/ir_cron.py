...
class attendanceAttendanceIrCron(models.TransientModel):
    _description = '钉钉考勤自动任务'
    _name = 'attendance.attendance.ir.cron'
    ...
    def start_yun_attendance_cron(self):
        ...
        :return:
        """
        logging.info(">>>---------------Start attendance Attendance Cron------------")
        # 自动拉取当天的下2天的排班信息
        today = datetime.date.today()
        tomorrow = today + datetime.timedelta(days=2)
        try:
            self.env['hr.attendance.plan.tran'].sudo().start_pull_plan_lists(today, tomorrow)
        ...
        logging.info(">>>---------------END attendance Attendance Cron--------------")
