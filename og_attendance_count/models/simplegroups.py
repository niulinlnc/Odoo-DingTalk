...
class HrEmployee(models.Model):
    ...
    din_group_id = fields.Many2one(comodel_name='attendance.simple.groups', string=u'考勤组', index=True)


class HrAttendanceSimpleGroups(models.Model):
    _name = 'hr.attendance.simple.groups'
    _rec_name = 'name'
    _description = '考勤组'
    ...
    group_id = fields.Char(string='考勤组ID', index=True)
    ...
    list_ids = fields.One2many(comodel_name='attendance.simple.groups.list', inverse_name='simple_id', string=u'班次列表')
        ...
        try:
            ...
            if result.get('ding_open_errcode') == 0:
                ...
                for group in result.get('groups').get('at_group_for_top_vo'):
                    ...
                    self_group = self.env['attendance.simple.groups'].search([('group_id', '=', group.get('group_id'))])
                    if self_group:
                        for group_list in self_group.list_ids:
                            group_list.sudo().unlink()
                        self_group.sudo().write(data)
                    else:
                        self.env['attendance.simple.groups'].sudo().create(data)
            else:
        ...
        for emp in emps:
            try:
                ...
                if result.get('ding_open_errcode') == 0:
                    ...
                    groups = self.env['attendance.simple.groups'].sudo().search([('group_id', '=', res.get('group_id'))])
                    if groups:
                        self._cr.execute(
...
class HrAttendanceSimpleGroupsList(models.Model):
    _description = "班次列表"
    _name = 'hr.attendance.simple.groups.list'
    _rec_name = 'class_name'

    simple_id = fields.Many2one(comodel_name='attendance.simple.groups', string=u'考勤组', index=True)
    ...
    # sections
    time_ids = fields.One2many(comodel_name='attendance.simple.groups.list.time', inverse_name='list_id', string=u'打卡时间段')


class HrattendanceSimpleGroupsListTime(models.Model):
    _description = "班次打卡时间段"
    _name = 'hr.attendance.simple.groups.list.time'
    _rec_name = 'list_id'

    list_id = fields.Many2one(comodel_name='attendance.simple.groups.list', string=u'班次', index=True)
