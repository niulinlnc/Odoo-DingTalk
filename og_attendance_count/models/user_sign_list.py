...
class attendanceSignList(models.Model):
    _name = 'attendance.signs.list'
    _description = "签到记录列表"
    _rec_name = 'emp_id'
    ...
    def get_signs_by_user(self, userid, signtime):
        ...
        url, token = self.env['attendance.parameter'].get_parameter_value_and_token('get_user_checkin')
        data = {
            'userid_list': userid,
        ...
        try:
            ...
            if result.get('errcode') == 0:
                ...
                for data in r_result.get('page_list'):
                    ...
                    self.env['attendance.signs.list'].create({
