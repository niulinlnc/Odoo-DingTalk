...
class DinDinCalendarEvent(models.Model):
    ...
    def create(self, values):
        if not values['number']:
            values['number'] = self.env['ir.sequence'].sudo(
            ).next_by_code('calendar.event.number')
    ...
    def create_dindin_calendar(self, val):
        ...
        end_time = datetime.datetime.strptime("{}.42".format(
            str(val.get('stop'))), "%Y-%m-%d %H:%M:%S.%f").timetuple()
        ...
        create_vo = {
            ...
            # 地点地址
            'location': val.get('location') if val.get('location') else '',
            'receiver_userids': userids,  # 接收人列表string
            'end_time': {
                # 结束的unix时间戳 (单位:毫秒)
                'unix_timestamp': "{}000".format(int(time.mktime(end_time))),
                'timezone': 'Shanghai',   # 时区
            },
            'calendar_type': 'notification',  # 提醒类型
            'start_time': {
                # 开始的unix时间戳
                'unix_timestamp': "{}000".format(int(time.mktime(start_time))),
                'timezone': 'Shanghai',  # 时区
            },
            'source': {
                'title': 'OdooERP',
            ...
            # 创建人uid
            'creator_userid': employee[0].din_id if employee[0].din_id else '',
    ...
    def unlink(self):
        for res in self:
            ...
            userid = self.env['hr.employee'].sudo().search(
